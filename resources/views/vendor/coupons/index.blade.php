...
<div style="background: white; border: 1px solid var(--border); border-radius: 0.75rem; overflow: hidden;">
    <div style="padding: 1.5rem; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center;">
        ...
        <a href="{{ route('vendor.coupons.create') }}" style="background: var(--primary); color: white; padding: 0.6rem 1.25rem; border-radius: 0.5rem; text-decoration: none; font-weight: 600; display: flex; align-items: center; gap: 0.5rem;">
    ...
    <table style="width: 100%; border-collapse: collapse; text-align: left;">
        <thead>
            <tr style="background: #f8fafc; border-bottom: 1px solid var(--border);">
                ...
                <th style="padding: 1rem 1.5rem; font-size: 0.75rem; font-weight: 600; color: var(--text-muted); text-transform: uppercase;">Property</th>
        ...
        <tbody>
            ...
            <tr style="border-bottom: 1px solid var(--border);">
                ...
                <td style="padding: 1rem 1.5rem;">
                    @if($coupon->property)
                        <span style="background: #e0f2fe; color: #0369a1; padding: 0.25rem 0.5rem; border-radius: 0.25rem; font-size: 0.875rem; font-weight: 600;">{{ $coupon->property->name }}</span>
                    @else
                ...
                <td style="padding: 1rem 1.5rem;">
                    <div style="display: flex; gap: 0.5rem;">
                        <a href="{{ route('vendor.coupons.edit', $coupon) }}" style="padding: 0.4rem; background: #f1f5f9; color: #475569; border-radius: 0.375rem; border: 1px solid #e2e8f0; display: inline-flex; align-items: center; justify-content: center;">
                        ...
                        <form action="{{ route('vendor.coupons.destroy', $coupon) }}" method="POST" onsubmit="return confirm('Delete this coupon?')" style="display: inline;">
                            @csrf @method('DELETE')
