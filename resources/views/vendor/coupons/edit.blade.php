...
<div style="max-width: 800px;">
    <div style="background: white; border: 1px solid var(--border); border-radius: 0.75rem; overflow: hidden;">
        ...
        <form action="{{ route('vendor.coupons.update', $coupon) }}" method="POST" style="padding: 2rem;">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="property_id" required style="width: 100%; padding: 0.75rem; border: 1px solid var(--border); border-radius: 0.5rem; background: white;">
                        ...
                        @foreach($properties as $property)
                            <option value="{{ $property->id }}" {{ old('property_id', $coupon->property_id) == $property->id ? 'selected' : '' }}>{{ $property->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="user_id" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border); border-radius: 0.5rem; background: white;">
                        ...
                        @foreach($customers as $customer)
                            <option value="{{ $customer->id }}" {{ old('user_id', $coupon->user_id) == $customer->id ? 'selected' : '' }}>{{ $customer->name }} ({{ $customer->email }})</option>
                        @endforeach
                    </select>
                </div>
            </div>
