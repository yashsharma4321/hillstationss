...
<div class="content-card">
    <div class="card-header">
        <h2 style="font-size: 1.125rem; font-weight: 600;">Category Details</h2>
        <a href="{{ route('admin.categories.index') }}" style="color: var(--secondary); text-decoration: none; font-size: 0.875rem;">← Back to List</a>
    </div>

    <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" style="padding: 2rem; max-width: 800px;">
        @csrf
        ...
        <div style="margin-bottom: 2rem;">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <select name="category_group" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border); border-radius: 0.5rem; font-family: inherit; font-size: 0.875rem;">
                    ...
                    <option value="BY AMENITIES" {{ old('category_group') == 'BY AMENITIES' ? 'selected' : '' }}>BY AMENITIES</option>
                    <option value="BY EXPERIENCE" {{ old('category_group') == 'BY EXPERIENCE' ? 'selected' : '' }}>BY EXPERIENCE</option>
                    <option value="BY GROUP SIZE" {{ old('category_group') == 'BY GROUP SIZE' ? 'selected' : '' }}>BY GROUP SIZE</option>
                    <option value="BY BUDGET" {{ old('category_group') == 'BY BUDGET' ? 'selected' : '' }}>BY BUDGET</option>
                    <option value="PET FRIENDLY" {{ old('category_group') == 'PET FRIENDLY' ? 'selected' : '' }}>PET FRIENDLY</option>
                </select>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <p style="font-size: 0.75rem; color: var(--text-muted); margin-top: 0.25rem;">Upload a small icon or representative image (JPG, PNG, SVG).</p>
                @error('icon')
                    <p style="color: var(--danger); font-size: 0.75rem; margin-top: 0.25rem;">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div style="margin-bottom: 1.5rem;">
                <label style="display: flex; align-items: center; gap: 0.75rem; font-size: 0.875rem; font-weight: 500; color: var(--text-main); cursor: pointer;">
                    <input type="checkbox" name="is_best_view" value="1" {{ old('is_best_view') ? 'checked' : '' }} 
                        style="width: 1.25rem; height: 1.25rem; accent-color: var(--primary);">
                    <span>Mark as Best View</span>
                </label>
                <p style="font-size: 0.75rem; color: var(--text-muted); margin-left: 2rem;">Categories marked as "Best View" will be highlighted on the frontend.</p>
            </div>
        ...
        <div style="display: flex; gap: 1rem; justify-content: flex-end; border-top: 1px solid var(--border); padding-top: 1.5rem;">
            ...
            <button type="submit" style="padding: 0.75rem 2rem; background: var(--primary); border: none; color: white; border-radius: 0.5rem; font-weight: 600; cursor: pointer;">Save Category</button>
        </div>
    </form>
</div>
@endsection
