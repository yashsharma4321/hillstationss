...
<div class="content-card">
    <div class="card-header" style="background: white;">
        <div>
            <h2 style="font-size: 1.25rem; font-weight: 700; color: #0f172a;">States</h2>
            <p style="font-size: 0.875rem; color: var(--text-muted); margin-top: 0.25rem;">Manage states for destinations.</p>
        </div>
            ...
            <table style="min-width: 900px; width: 100%;">
                <thead>
                    <tr style="background: #f8fafc;">
                        <th style="border-bottom: 1px solid var(--border); width: 80px; text-align: left; padding: 1rem 1.5rem;">ID</th>
                        <th style="border-bottom: 1px solid var(--border); text-align: left; padding: 1rem 1.5rem;">Name</th>
                        <th style="border-bottom: 1px solid var(--border); text-align: left; padding: 1rem 1.5rem;">Slug</th>
                        <th style="border-bottom: 1px solid var(--border); text-align: left; padding: 1rem 1.5rem;">Status</th>
                        <th style="border-bottom: 1px solid var(--border); text-align: right; padding: 1rem 1.5rem;">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($states as $state)
                        <tr class="dest-card" style="transition: background 0.2s;">
                            <td style="padding: 1.25rem 1.5rem; font-weight: 600; color: #94a3b8;">#{{ $state->id }}</td>
                            <td style="padding: 1.25rem 1.5rem;">
                                <div style="font-weight: 700; color: #1e293b;">{{ $state->name }}</div>
                            </td>
                            ...
                            <td style="padding: 1.25rem 1.5rem; text-align: right;">
                                <div style="display: inline-flex; gap: 0.5rem; align-items: center; justify-content: flex-end; width: 100%;">
                                    <a href="{{ route('admin.states.edit', $state) }}" class="action-btn" title="Edit State">
                                    ...
                                    <form action="{{ route('admin.states.destroy', $state) }}" method="POST" style="margin: 0;" onsubmit="return confirm('Delete this state?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="action-btn delete" title="Delete State">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" style="text-align: center; padding: 4rem 1.5rem;">
                                <p style="color: var(--text-muted);">No states found.</p>
                                <a href="{{ route('admin.states.create') }}" style="color: var(--primary); font-weight: 600; text-decoration: none;">Add one now</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
