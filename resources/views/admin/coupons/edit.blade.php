...
<div style="max-width: 800px;">
    <div style="background: white; border: 1px solid var(--border); border-radius: 0.75rem; overflow: hidden;">
        <div style="padding: 1.5rem; border-bottom: 1px solid var(--border);">
            <h3 style="font-size: 1rem; font-weight: 700; color: var(--text-main);">Update Coupon Details</h3>
        </div>

        <form action="{{ route('admin.coupons.update', $coupon) }}" method="POST" style="padding: 2rem;">
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
                <div>
                    ...
                    <input type="number" name="value" value="{{ old('value', $coupon->value) }}" required step="0.01"
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <textarea name="description" rows="3" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border); border-radius: 0.5rem;">{{ old('description', $coupon->description) }}</textarea>
            </div>

            <div style="display: flex; gap: 2rem; margin-bottom: 2rem;">
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; font-weight: 600; font-size: 0.875rem;">
                    <input type="checkbox" name="is_global" value="1" {{ old('is_global', $coupon->is_global) ? 'checked' : '' }} style="width: 1.25rem; height: 1.25rem; accent-color: var(--primary);">
                    <span>Show as Global Coupon (Visible to everyone)</span>
                </label>
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; font-weight: 600; font-size: 0.875rem;">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $coupon->is_active) ? 'checked' : '' }} style="width: 1.25rem; height: 1.25rem; accent-color: var(--primary);">
                    <span>Is Active</span>
                </label>
            </div>

            <div style="display: flex; gap: 1rem; justify-content: flex-end;">
                <a href="{{ route('admin.coupons.index') }}" style="padding: 0.75rem 1.5rem; background: #f1f5f9; color: #475569; border-radius: 0.5rem; text-decoration: none; font-weight: 600;">Cancel</a>
                <button type="submit" style="padding: 0.75rem 2.5rem; background: var(--primary); color: white; border: none; border-radius: 0.5rem; font-weight: 700; cursor: pointer;">Update Coupon</button>
            </div>
        </form>
    </div>
</div>
