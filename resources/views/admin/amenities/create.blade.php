...
@section('header', 'Add New Amenity')

@section('content')
<div class="content-card">
    <div class="card-header">
        <h2 style="font-size: 1.125rem; font-weight: 600;">Amenity Details</h2>
        <a href="{{ route('admin.amenities.index') }}" style="color: var(--secondary); text-decoration: none; font-size: 0.875rem;">← Back to List</a>
    </div>

    <form action="{{ route('admin.amenities.store') }}" method="POST" enctype="multipart/form-data" style="padding: 2rem; max-width: 800px;">
        @csrf
        ...
        <div style="margin-bottom: 2rem;">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <p style="font-size: 0.75rem; color: var(--text-muted); margin-top: 0.25rem;">Upload a small icon (Best size: 32x32 px).</p>
                @error('icon')
                    <p style="color: var(--danger); font-size: 0.75rem; margin-top: 0.25rem;">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div style="display: flex; gap: 1rem; justify-content: flex-end; border-top: 1px solid var(--border); padding-top: 1.5rem;">
            ...
            <button type="submit" style="padding: 0.75rem 2rem; background: var(--primary); border: none; color: white; border-radius: 0.5rem; font-weight: 600; cursor: pointer;">Save Amenity</button>
        </div>
    </form>
</div>
@endsection
