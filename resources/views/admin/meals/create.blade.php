...
@section('header', 'Add New Meal')

@section('content')
<div class="content-card">
    <div class="card-header">
        <h2 style="font-size: 1.125rem; font-weight: 600;">Meal Details</h2>
        <a href="{{ route('admin.meals.index') }}" style="color: var(--secondary); text-decoration: none; font-size: 0.875rem;">← Back to List</a>
    </div>

    <form action="{{ route('admin.meals.store') }}" method="POST" style="padding: 2rem; max-width: 800px;">
        @csrf
        ...
        <div style="margin-bottom: 2rem;">
            ...
            <div style="margin-bottom: 1.5rem;">
                <label style="display: flex; align-items: center; font-size: 0.875rem; font-weight: 500; color: var(--text-main); cursor: pointer;">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} style="margin-right: 0.5rem; width: 1.1rem; height: 1.1rem;">
                    Active Status
                </label>
            </div>
        </div>
        ...
        <div style="display: flex; gap: 1rem; justify-content: flex-end; border-top: 1px solid var(--border); padding-top: 1.5rem;">
            ...
            <button type="submit" style="padding: 0.75rem 2rem; background: var(--primary); border: none; color: white; border-radius: 0.5rem; font-weight: 600; cursor: pointer;">Save Meal</button>
        </div>
    </form>
</div>
@endsection
