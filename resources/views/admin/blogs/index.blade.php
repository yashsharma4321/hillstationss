...
@section('styles')
<style>
    .blog-card:hover { background-color: #f8fafc; }
    .action-btn {
        width: 32px;
        height: 32px;
        display: flex;
...
<div class="content-card">
    <div class="card-header" style="background: white;">
        <div>
            <h2 style="font-size: 1.25rem; font-weight: 700; color: #0f172a;">Blog Posts</h2>
            <p style="font-size: 0.875rem; color: var(--text-muted); margin-top: 0.25rem;">Create and manage articles for your website blog.</p>
        </div>
            ...
            <table style="min-width: 900px;">
                <thead>
                    <tr style="background: #f8fafc;">
                        <th style="border-bottom: 1px solid var(--border);">Blog Details</th>
                        <th style="border-bottom: 1px solid var(--border);">Status</th>
                        <th style="border-bottom: 1px solid var(--border);">Published Date</th>
                        <th style="border-bottom: 1px solid var(--border); text-align: right;">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($blogs as $blog)
                        <tr class="blog-card" style="transition: background 0.2s;">
                            <td style="padding: 1.25rem 1.5rem;">
                                <div style="display: flex; align-items: center; gap: 1rem;">
                                    @if($blog->image)
                                        <img src="{{ Storage::url($blog->image) }}" style="width: 50px; height: 50px; object-fit: cover; border-radius: 0.5rem; background: #f1f5f9;">
                                    @else
                                    ...
                                    @endif
                                    <div>
                                        <div style="font-weight: 700; color: #1e293b;">{{ $blog->title }}</div>
                                        <div style="font-size: 0.75rem; color: var(--text-muted); margin-top: 0.25rem;">{{ Str::limit($blog->description, 60) }}</div>
                                    </div>
                                </div>
                            </td>
                            <td style="padding: 1.25rem 1.5rem;">
                                @if($blog->status)
                                    <span style="padding: 0.25rem 0.75rem; background: #dcfce7; color: #166534; border-radius: 9999px; font-size: 0.75rem; font-weight: 600; border: 1px solid #bbf7d0;">● Public</span>
                                @else
                                    <span style="padding: 0.25rem 0.75rem; background: #f1f5f9; color: #475569; border-radius: 9999px; font-size: 0.75rem; font-weight: 600; border: 1px solid #e2e8f0;">● Draft</span>
                                @endif
                            </td>
                            ...
                            <td style="padding: 1.25rem 1.5rem; text-align: right;">
                                <div style="display: inline-flex; gap: 0.5rem;">
                                    <a href="{{ route('admin.blogs.edit', $blog) }}" class="action-btn" title="Edit Blog">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" style="text-align: center; padding: 4rem 1.5rem;">
                                ...
                                <a href="{{ route('admin.blogs.create') }}" style="color: var(--primary); font-weight: 600; text-decoration: none;">Create your first post</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
